...
\texttt{\seqsplit{notification-service}} & Node.js + Express & Notificaciones multicanal simuladas \\
\texttt{report-service} & Java 21 + Spring Boot & Modelo de lectura CQRS, reportes \\
\texttt{\seqsplit{telemetry-service}} & Java 21, sockets TCP + gRPC & Canal PE-U1: equipos del abonado + latidos de nodo, reloj de Lamport (Sección~\ref{sec:pe-u1-correl}) \\
...
\texttt{domain} & \texttt{Ticket}, puertos, políticas & Cero anotaciones Spring/JPA; se prueba con JUnit puro \\
...
\texttt{ai-service}, \texttt{\seqsplit{notification-service}}, \texttt{api-gateway} & Sin cambios &
...
\begin{enumerate}[nosep]
  \item \textbf{Repository} --- desacopla el dominio de \texttt{\seqsplit{JpaRepository}} (Spring Data) vía
  ...
  \item \textbf{Chain of Responsibility} --- dos eslabones de evaluación de escalado
      (\texttt{\seqsplit{SlaBreachedEscalationHandler}}, \texttt{\seqsplit{StaleCriticalEscalationHandler}}) que
