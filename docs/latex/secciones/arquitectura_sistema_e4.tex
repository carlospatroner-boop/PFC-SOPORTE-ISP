...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{../diagrams/c4-nivel3-componentes.png}
\caption{C4 Nivel 3 — componentes de \texttt{ticket-service}, Entrega 4.}
\label{fig:c4-nivel3}
\end{figure}
...
encontró además una discrepancia en la Tabla~\ref{tab:capas-e4}: dice que \texttt{domain} tiene
``cero anotaciones Spring/JPA''. Verificado con \texttt{grep} sobre el paquete: es falso para la
...
\texttt{domain}. Se corrige la afirmación en la Sección~\ref{sec:hexagonal-e4}; sacar esas doce
líneas hacia clases \texttt{@Configuration} explícitas en \texttt{infrastructure} queda como
trabajo pendiente, no resuelto por este diagrama.
...
\texttt{domain} & \texttt{Ticket}, puertos, políticas & Cero JPA; 12 líneas de anotación Spring
  (\texttt{@Component}/\texttt{@Value}/\texttt{@Order} para que Spring descubra cada
  implementación de un puerto) en 9 de sus clases, ver Sección~\ref{sec:c4-nivel3}; se prueba con
  JUnit puro \\
...
había aplicado a \texttt{ticket-service}: los otros cinco microservicios seguían con la
organización heredada de la Entrega 2. Se extendió el mismo patrón a \texttt{auth-service} y
\texttt{report-service} --los de menor superficie de los cinco restantes-- dejando
explícitamente fuera de alcance \texttt{ai-service}, \texttt{notification-service} y
\texttt{api-gateway} por el tiempo disponible antes del cierre; se documenta el alcance real en
vez de sugerir un refactor completo que no ocurrió.
