...
de las líneas según el historial de control de versiones (\texttt{git log --numstat}, excluyendo
explícitamente dependencias y artefactos generados —por ejemplo \texttt{node\_modules/}, que en
un commit aislado de julio infla artificialmente el recuento crudo del backend en más de 226\,000
líneas si no se descarta—). Esta tabla no reemplaza al historial: quien quiera verificarla puede
correr el mismo comando sobre las mismas rutas.
...
\midrule
Backend (\texttt{services/}) & C. Pacheco 41\,\%, C. Carpio 27\,\%, R. Cando 25\,\%, J. Álvarez 7\,\% \\
Aplicación web (\texttt{apps/web/}) & C. Carpio 98\,\% \\
Aplicación móvil (\texttt{apps/mobile/}) & R. Cando 95\,\% \\
Manuscrito (\texttt{docs/latex/}) & C. Pacheco 75.24\,\%, R. Cando 11.87\,\%, C. Carpio 10.77\,\%, J. Álvarez 2.12\,\% \\
Experimentos (\texttt{experimentos/}, \texttt{spark/}) & R. Cando 76\,\%, C. Pacheco 18\,\% \\
Observabilidad (\texttt{ops/}) & C. Pacheco 100\,\% \\
CI/CD (\texttt{.github/}) & C. Carpio 71\,\%, J. Álvarez 17\,\% \\
Arquitectura (\texttt{docs/adr/}, \texttt{docs/diagrams/}) & \textbf{J. Álvarez 46\,\%}, C. Pacheco 23\,\% \\
Contratos (\texttt{pacts/}, OpenAPI) & \textbf{J. Álvarez 100\,\%} \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize\textit{Nota: porcentajes calculados con \texttt{git log --numstat} sobre cada
ruta indicada, tomados en el commit \texttt{6560409}. Ediciones posteriores a
\texttt{docs/latex/} pueden desactualizar solo la fila de Manuscrito.}}
\end{table}
